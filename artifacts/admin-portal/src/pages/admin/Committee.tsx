    ...
    <AdminLayout>
            ...
            return (
              <div key={m.id} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
                <div className="h-20 overflow-hidden flex-shrink-0">
                  <MemberAvatar photoUrl={m.photoUrl} initials={m.initials} />
                </div>
                <div className="p-3 flex-1 flex flex-col gap-1">
                  ...
                  {m.subcommitteeName && (
                    <p className="text-[11px] text-gray-400">{m.subcommitteeName}</p>
                  )}
                ...
                <div className="flex gap-2 px-3 pb-3">
                  <button
                    onClick={() => openEdit(m)}
                    className="flex-1 flex items-center justify-center gap-1 py-1.5 text-[12px] font-medium rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50"
                  >
                    <Pencil className="w-3 h-3" /> Edit
                  </button>
                  <button
                    onClick={() => setDeleteId(m.id)}
                    className="flex items-center justify-center px-2.5 py-1.5 rounded-lg border text-red-600 border-red-100 hover:bg-red-50"
                  >
      ...
      {showModal && (
        ...
        >
          ...
          <InputField label="Committee Level" required error={errors.committeeLevel}>
            <select
          ...
          <InputField label="Photo" error={undefined}>
            <ImageUploadField
              value={form.photoUrl}
              onChange={(path) => set("photoUrl", path)}
              accept="image/png,image/jpeg,image/webp"
              hint="Optional — PNG, JPG, or WebP portrait photo"
            />
          </InputField>
        </Modal>
      )}
