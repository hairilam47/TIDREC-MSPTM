...
export function ImageUploadField({ value, onChange, accept = "image/*", hint }: ImageUploadFieldProps) {
  ...
  React.useEffect(() => {
    if (!value) { setPreviewSrc(null); return; }
    if (value.startsWith("http://") || value.startsWith("https://")) { setPreviewSrc(value); return; }
    if (value.startsWith("/objects/")) {
      const token = localStorage.getItem("satbds_token");
      const stripped = value.slice("/objects/".length);
      let objectUrl: string | null = null;
      fetch(`${API}/storage/objects/${stripped}`, { headers: { Authorization: `Bearer ${token}` } })
    ...
    if (value.startsWith("/api/")) { setPreviewSrc(value); return; }
    setPreviewSrc(null);
  }, [value]);
  ...
  const handleRemove = () => { onChange(""); setPreviewSrc(null); setError(null); };
    ...
    <div className="flex flex-col gap-2">
      ...
      {previewSrc ? (
        <div className="flex items-center gap-3">
          <div className="w-16 h-16 rounded-lg border flex items-center justify-center overflow-hidden bg-gray-50 flex-shrink-0" style={{ borderColor: "#dee2e6" }}>
            <img src={previewSrc} alt="Preview" className="w-full h-full object-contain" />
          </div>
          <div className="flex flex-col gap-1.5">
            <button type="button" onClick={() => inputRef.current?.click()} disabled={uploading}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[13px] font-medium border disabled:opacity-50"
        ...
        <button type="button" onClick={() => inputRef.current?.click()} disabled={uploading}
          className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-lg border-2 border-dashed text-[13px] font-medium transition-colors disabled:opacity-50 hover:border-[#0E6E74] hover:text-[#0E6E74]"
          style={{ borderColor: "#dee2e6", color: "#6c757d" }}>
          {uploading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Upload className="w-4 h-4" />}
          {uploading ? "Uploading…" : "Click to upload image"}
        </button>
      )}
      {hint && !error && <p className="text-[12px]" style={{ color: "#6c757d" }}>{hint}</p>}
      {error && <p className="text-[12px]" style={{ color: "#dc3545" }}>{error}</p>}
    </div>
  );
}
